...
- Reach for `layout: composed` whenever the beat has a form. A composed slide carries an optional `Title:` and one or more `Block:` lines. Named shortcuts cover the common shapes — `stat-row`, `card-grid`, `comparison`, `process`, `timeline`, `tree` (an org chart / decomposition from an indented list), and `icon-list` (icons as bullets) — and `freeform` places token-bound boxes, text, arrows, and icons on the grid for anything else. On-brand line **icons** are available too: as an `icon-list`, a `[icon-name]` prefix on a card or tree node, or in freeform. Every block is drawn as token-locked shapes that pass a mechanical lint, so none can go off-brand. Prefix the one card / winning panel / turning-point milestone / lead node with `!` to make it lead. Blocks stack, or place them with `at cols 1-6` / `at left`.
...
When the spec is written, name the file and point the user at `build-deck` (via `$skill build-deck` or `/skills`) to render it, or `slop-check` (via `$skill slop-check` or `/skills`) to audit it first.
