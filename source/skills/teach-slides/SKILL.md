...
**`brand.json`** is what the renderer reads. Write `template` as `.slides/template.pptx`, the path relative to the project root, so `render.py` resolves it when run from there.

```json
{
  "template": ".slides/template.pptx",
