...
**T-040 (the source-material assessment) is COMPLETE.** James and Claude reviewed the 8 source files plus the Graphic Continuum collaboratively, including the visual pass. The assessment produced three committed repo artifacts that supersede this plan's draft guidance on craft content:

- `notes/source-review.md` — the source assessment: a section per source file, principle-to-reference-doc mapping, James's anti-patterns, and eight logged decisions (incl. "principle-only craft references — strip all named examples", "Bluey leads", "earned vs unearned emotion", "register matters", "concept diagrams first-class", "prose slop adopts stop-slop").
- `notes/approach.md` — the distilled, principle-only method: Plan/Create/Deliver, register, visual craft, data/charts/diagrams, the slop line.
- `notes/slop-detector.md` — the two-layer slop detector spec (Layer 1 presentation slop from James's craft; Layer 2 prose slop adopted from `stop-slop`, Hardik Pandya, MIT). This is the spec for `presentation-craft/reference/slop.md`.

**These three notes ARE the authority for all craft content.** Wave 2 reference files (T-051..T-055) are written by transcribing and structuring these notes — NOT by re-distilling source files and NOT by inventing craft (D-008 forbids autonomous distillation; the distillation is already done). Every craft reference file is principle-only: no named examples (Wall-E, TOMS, JFK, Bluey-as-IP, the Third Silesian War, etc.) appear in shipped skill files.

**Workflow note (added by `/build`).** This plan is adopted verbatim as the `/build` workflow plan for slug `slides-pack`. The execution manifest below is canonical. T-040 is marked complete in `slides-pack-state.md` and is skipped during implementation. All other workstreams (A, C, D, E) now proceed.

### Workflow artifacts (`/build` phase map)
...
- Phase 2 Review reads the four above; writes `slides-pack-review.md`.
