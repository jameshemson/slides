...
Tell the user where the `.pptx` was written. To audit the finished deck, point them at `slop-check` (via the skill tool). To change it later, point them at `revise` (via the skill tool).
